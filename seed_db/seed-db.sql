...
-----------------------------------------------------------
-- ACCESS SETTINGS (ROLE & PERMISSION)
-----------------------------------------------------------
...
-----------------------------------------------------------
-- BLOCKCHAIN CERTIFICATES MODULE (MENU + API)
-----------------------------------------------------------

-- 1) Inserir as telas no menu (screen + menu-screen)
INSERT INTO access_controls(name, path, icon, parent_path, hierarchy_id, type, method) VALUES
('Certificates', 'certificates_parent', 'certificates.svg', NULL, 8, 'menu-screen', NULL),
('Issue Certificate', 'certificates/issue', NULL, 'certificates_parent', 1, 'menu-screen', NULL),
('Verify Certificate', 'certificates/verify', NULL, 'certificates_parent', 2, 'menu-screen', NULL);

-- 2) Inserir as APIs que o FRONT usa
INSERT INTO access_controls(name, path, icon, parent_path, hierarchy_id, type, method) VALUES
('API - Issue Certificate',  '/api/v1/certificates/issue',  NULL, 'certificates_parent', NULL, 'api', 'POST'),
('API - Verify Certificate', '/api/v1/certificates/verify', NULL, 'certificates_parent', NULL, 'api', 'GET');

-----------------------------------------------------------
-- PERMISSIONS (MAPEAR AS ROTAS PARA O ADMIN)
-----------------------------------------------------------

-- O admin é role_id = 1
-- Para cada access_control, criamos uma permissão correspondente
